...
pub fn analyze(book: &OrderBookSnapshot, trades: &[TradeTick]) -> MicrostructureFeatures {
    ...
    let bid_depth_usd = book.bid_depth.iter().map(|(p,q)| p*q).sum();
    let ask_depth_usd = book.ask_depth.iter().map(|(p,q)| p*q).sum();
    ...
    let trade_volume_usd: f64 = trades.iter().map(|t| t.price*t.quantity).sum();
    let buy_usd: f64 = trades.iter().filter(|t| t.side == "buy").map(|t| t.price*t.quantity).sum();
    ...
    let liquidity_score = (100.0 / (1.0 + spread_bps.max(0.0))) * (1.0 + (total_depth.max(0.0).log10()/8.0).clamp(0.0, 1.0)) / (1.0 + i10 / 10.0);
