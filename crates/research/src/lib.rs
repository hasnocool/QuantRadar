...
pub fn events(x:&AssetSnapshot)->Vec<Event>{let r=&x.row;let mut e=Vec::new();if r.new_high_20{e.push(Event{ts:r.ts,symbol:x.symbol.clone(),kind:"NEW_HIGH".into(),severity:0.8,details:"Close exceeded prior 20-period high".into()});}if r.new_low_20{e.push(Event{ts:r.ts,symbol:x.symbol.clone(),kind:"NEW_LOW".into(),severity:0.8,details:"Close fell below prior 20-period low".into()});}if r.volume_z_20.unwrap_or(0.0)>3.0{e.push(Event{ts:r.ts,symbol:x.symbol.clone(),kind:"VOLUME_ANOMALY".into(),severity:(r.volume_z_20.unwrap_or(0.0)/5.0).min(1.0),details:"Volume z-score exceeded 3".into()});}if r.atr_pct.unwrap_or(0.0)>0.15{e.push(Event{ts:r.ts,symbol:x.symbol.clone(),kind:"VOLATILITY_SPIKE".into(),severity:1.0,details:"ATR as percent of price exceeded 15%".into()});}e}
