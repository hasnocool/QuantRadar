...
// ============================================================================
// CLI Definition (same as before but with working handlers)
// ============================================================================
...
async fn handle_data(cmd: DataCommand) -> Result<()> {
    match cmd {
        DataCommand::Import { source, format, output } => {
            ...
            // Real implementation would read from source and convert to format
        ...
        DataCommand::Export { dataset, format, output } => {
            ...
            // Real implementation would query dataset and write
        ...
        DataCommand::Ingest { exchange, symbols, duration } => {
            ...
            // Real implementation would start websocket ingestion
        ...
        DataCommand::Validate { dataset, checks } => {
            ...
            // Real implementation would run validation
    ...
    match cmd {
        AnalyzeCommand::Discover { exchange, output } => {
            ...
            if let Some(out) = output {
                write_json(&PathBuf::from(out), &markets)?;
        ...
        AnalyzeCommand::Scan { interval, limit, concurrency, quote, output } => {
            ...
            while let Some(j) = jobs.next().await {
                ...
                if let Ok(bars) = bars {
                    ...
                    if let Some(last) = rows.last() {
                        ...
                        signals.extend(run_default(&rows, regime));
                    }
                }
            }
            ...
            if let Some(out) = output {
                write_json(&PathBuf::from(out), &result)?;
        ...
        AnalyzeCommand::Screen { symbol, interval, output } => {
            ...
            let result = ScreenResult {
                generated_at: Utc::now(),
                regime: regime.clone(),
                signals: run_default(&rows, regime),
            };
        ...
        AnalyzeCommand::Features { symbols, output } => {
            ...
            let client = KrakenClient::default();
            // Real implementation would fetch and process
        ...
        AnalyzeCommand::Rank { universe, interval, top_n } => {
            ...
            // Real implementation would rank
        ...
        AnalyzeCommand::Pca { symbols, interval, components } => {
            ...
            // Real implementation would run PCA
...
async fn handle_research(cmd: ResearchCommand) -> Result<()> {
    match cmd {
        ResearchCommand::Backtest { strategy, dataset, cash, output } => {
            ...
            // Real implementation would load data and run backtest
            let bars_path = PathBuf::from(format!("data/{}", dataset));
            if bars_path.exists() {
