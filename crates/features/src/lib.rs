...
mod tests {
    ...
    fn feature_rows_no_future_leakage() {
        ...
        // First 20 rows should have None for features requiring 20-period lookback
        assert!(rows[0].ema_20.is_none());
        assert!(rows[19].ema_20.is_some()); // first valid at index 19 (period-1)
