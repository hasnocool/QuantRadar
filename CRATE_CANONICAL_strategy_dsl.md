# CRATE_CANONICAL_strategy_dsl.md — quantaradar-strategy_dsl

## quantaradar-strategy_dsl

### Purpose
Standalone crate `quantaradar-strategy_dsl` in the QuantRadar workspace (`crates/standalone/strategy_dsl`).
# crates/standalone/strategy_dsl/codemap

### Version / License / Edition
- version.workspace = true (workspace 0.2.0), edition.workspace = true (2024), license.workspace = true (MIT), rust-version 1.85

### Full codemap (from crates/standalone/strategy_dsl/codemap.md)
```text
# crates/standalone/strategy_dsl/codemap

## Responsibility
Strategy DSL and contract definitions.

## Design
Strict Rust types, native computation, minimal speculation.

## Flow
Upstream core/features → strategy_dsl computation → downstream consumers.

## Integration
Used by workspace pipeline; depends on core/features.
```

### Source layout (`src/`)
- `src/lib.rs`

### Public API surface (sampled from source)
- `struct StrategyExpr (lib.rs)`
- `fn parse (lib.rs)`
- `fn evaluate (lib.rs)`

### Cargo manifest (abridged)
```toml
[package]
name = "quantaradar-strategy_dsl"
version.workspace = true
edition.workspace = true
rust-version.workspace = true
license.workspace = true

[dependencies]
anyhow.workspace = true
serde.workspace = true
serde_json.workspace = true
chrono.workspace = true
```

### Dependencies (manifest keys)
`name`

### Integration points
- Workspace member: `crates/standalone/strategy_dsl` (see root `Cargo.toml` members list).
- Shared vs standalone: `standalone` — domain/application-level crate built on shared infrastructure.
- See `ARCHITECTURE_CANONICAL.md` data-flow section and root `codemap.md` for cross-crate wiring.
- Verify: `cargo check -p quantaradar-strategy_dsl` / `cargo doc -p quantaradar-strategy_dsl --no-deps`.

### Verification checklist
- [ ] `cargo check -p quantaradar-strategy_dsl` passes
- [ ] `cargo doc -p quantaradar-strategy_dsl` renders public API
- [ ] codemap.md matches `src/` contents
- [ ] No unused-dependency warnings (`cargo machete` / `cargo udeps` optional)
