# CRATE_CANONICAL_multi_exchange.md — quantaradar-multi_exchange

## quantaradar-multi_exchange

### Purpose
Standalone crate `quantaradar-multi_exchange` in the QuantRadar workspace (`crates/standalone/multi_exchange`).
# crates/standalone/multi_exchange/codemap

### Version / License / Edition
- version.workspace = true (workspace 0.2.0), edition.workspace = true (2024), license.workspace = true (MIT), rust-version 1.85

### Full codemap (from crates/standalone/multi_exchange/codemap.md)
```text
# crates/standalone/multi_exchange/codemap

## Responsibility
Multi-exchange data aggregation.

## Design
Strict Rust types, native computation, minimal speculation.

## Flow
Upstream core/features → multi_exchange computation → downstream consumers.

## Integration
Used by workspace pipeline; depends on core/features.
```

### Source layout (`src/`)
- `src/lib.rs`

### Public API surface (sampled from source)
- `struct ExchangeRef (lib.rs)`
- `fn list (lib.rs)`

### Cargo manifest (abridged)
```toml
[package]
name = "quantaradar-multi_exchange"
version.workspace = true
edition.workspace = true
rust-version.workspace = true
license.workspace = true

[dependencies]
anyhow.workspace = true
serde.workspace = true
serde_json.workspace = true
chrono.workspace = true
```

### Dependencies (manifest keys)
`name`

### Integration points
- Workspace member: `crates/standalone/multi_exchange` (see root `Cargo.toml` members list).
- Shared vs standalone: `standalone` — domain/application-level crate built on shared infrastructure.
- See `ARCHITECTURE_CANONICAL.md` data-flow section and root `codemap.md` for cross-crate wiring.
- Verify: `cargo check -p quantaradar-multi_exchange` / `cargo doc -p quantaradar-multi_exchange --no-deps`.

### Verification checklist
- [ ] `cargo check -p quantaradar-multi_exchange` passes
- [ ] `cargo doc -p quantaradar-multi_exchange` renders public API
- [ ] codemap.md matches `src/` contents
- [ ] No unused-dependency warnings (`cargo machete` / `cargo udeps` optional)
