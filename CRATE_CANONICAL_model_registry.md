# CRATE_CANONICAL_model_registry.md — quantaradar-model_registry

## quantaradar-model_registry

### Purpose
Standalone crate `quantaradar-model_registry` in the QuantRadar workspace (`crates/standalone/model_registry`).
# crates/standalone/model_registry/codemap

### Version / License / Edition
- version.workspace = true (workspace 0.2.0), edition.workspace = true (2024), license.workspace = true (MIT), rust-version 1.85

### Full codemap (from crates/standalone/model_registry/codemap.md)
```text
# crates/standalone/model_registry/codemap

## Responsibility
Model registry and versioning.

## Design
Strict Rust types, native computation, minimal speculation.

## Flow
Upstream core/features → model_registry computation → downstream consumers.

## Integration
Used by workspace pipeline; depends on core/features.
```

### Source layout (`src/`)
- `src/lib.rs`

### Public API surface (sampled from source)
- `struct ModelEntry (lib.rs)`
- `fn register (lib.rs)`

### Cargo manifest (abridged)
```toml
[package]
name = "quantaradar-model_registry"
version.workspace = true
edition.workspace = true
rust-version.workspace = true
license.workspace = true

[dependencies]
anyhow.workspace = true
serde.workspace = true
serde_json.workspace = true
chrono.workspace = true
```

### Dependencies (manifest keys)
`name`

### Integration points
- Workspace member: `crates/standalone/model_registry` (see root `Cargo.toml` members list).
- Shared vs standalone: `standalone` — domain/application-level crate built on shared infrastructure.
- See `ARCHITECTURE_CANONICAL.md` data-flow section and root `codemap.md` for cross-crate wiring.
- Verify: `cargo check -p quantaradar-model_registry` / `cargo doc -p quantaradar-model_registry --no-deps`.

### Verification checklist
- [ ] `cargo check -p quantaradar-model_registry` passes
- [ ] `cargo doc -p quantaradar-model_registry` renders public API
- [ ] codemap.md matches `src/` contents
- [ ] No unused-dependency warnings (`cargo machete` / `cargo udeps` optional)
